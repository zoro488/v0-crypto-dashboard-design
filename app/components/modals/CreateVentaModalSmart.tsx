...
export function CreateVentaModal({ open, onClose, onSuccess }: CreateVentaModalProps) {
  ...
  const totales = React.useMemo(() => {
    ...
    // Bóveda Monte = precioCompra × cantidad (COSTO de mercancía)
    const totalBovedaMonte = carrito.reduce((acc, item) => acc + (item.precioCompra * item.cantidad), 0)
    
    // Fletes = precioFlete × cantidad (TRANSPORTE)
    const totalFletes = carrito.reduce((acc, item) => acc + (item.precioFlete * item.cantidad), 0)
  ...
  const agregarAlCarrito = (producto: {
    ...
    setCarrito(prev => {
      ...
      // Nuevo item con flete default de 500 MXN (editable)
      const FLETE_DEFAULT = 500
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Preparar datos para el servicio de transacciones
      const ventaData: VentaData = {
        ...
        metodoPago: metodoPago as 'efectivo' | 'tarjeta' | 'transferencia' | 'crypto',
        montoPagado: montoRealPagado,
        montoRestante: saldoPendiente,
      }

      // Usar servicio centralizado de transacciones atómicas
      // Esto maneja: validación stock, resta stock, crea venta, 
      // actualiza deuda cliente, y distribuye a 3 bancos
