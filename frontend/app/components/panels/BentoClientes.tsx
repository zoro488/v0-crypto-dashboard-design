...
export default function BentoClientes() {
  ...
  const totalCobrado = clientes?.reduce((acc, c) => acc + c.totalPagado, 0) || 0
  const clientesPendientes = clientes?.filter((c) => c.deudaTotal > 0).length || 0
                ...
                {topClientes.map((cliente, idx) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-4 text-right">
                      <div className="font-mono text-white">${(cliente.totalVentas / 1000).toFixed(0)}K</div>
                    </td>
                    <td className="py-4 px-4 text-right">
                      <div className="font-mono text-red-400">${(cliente.deudaTotal / 1000).toFixed(0)}K</div>
                    </td>
                    <td className="py-4 px-4 text-right">
                      <div className="font-mono text-green-400">${(cliente.totalPagado / 1000).toFixed(0)}K</div>
                    </td>
