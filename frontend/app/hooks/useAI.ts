...
export function useAI() {
  ...
  const generateInsights = useCallback(async (data: any): Promise<AIInsight[]> => {
    setIsLoading(true)
    ...
    try {
      ...
      // Stock analysis
      if (data.productos) {
        const lowStock = data.productos.filter((p: any) => p.stock < p.stockMinimo)
      ...
      // Sales analysis
      if (data.ventas) {
        const totalVentas = data.ventas.reduce((sum: number, v: any) => sum + v.total, 0)
        insights.push({
